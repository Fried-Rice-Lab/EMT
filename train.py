...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        ...
        if phase == 'train':
            dataset_enlarge_ratio = dataset_opt.get('dataset_enlarge_ratio', 1)#如果有就返回该值，没有就返回1
            train_sets = [build_dataset(dataset_opt)]
            datasets_used = f"\n\t\t{dataset_opt['name']}: {len(train_sets[0])} images" #得到一个字符串

            extra_datasets = dataset_opt.get('extra_datasets', None)
            if extra_datasets is not None:
                for _, extra_dataset in extra_datasets.items(): #对字典迭代
            ...
            logger.info('Training statistics:'   #在log中打印
        ...
        elif phase.split('_')[0] == 'val': #在_位置分割，0代表前面的
...
def train_pipeline(root_path):
    ...
    resume_state = load_resume_state(opt) #选择最大的.state
    ...
    copy_opt_file(args.expe, opt['path']['experiments_root'])
    copy_opt_file(args.task, opt['path']['experiments_root'])
    ...
    if prefetch_mode is None or prefetch_mode == 'cpu':
        prefetcher = CPUPrefetcher(train_loader) #提高训练效率
        ...
        while train_data is not None:
            ...
            if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0):
                for val_loader in val_loaders:
                    model.validation(val_loader, current_iter, tb_logger, opt['val']['save_val_img'])
