...
from archs.utils import MeanShift, Conv2d1x1, Conv2d3x3, ShiftConv2d1x1, PixelMixer, \
    SWSA, TransformerGroup, Upsampler, Swish
...
class MixedTransformerBlock(TransformerGroup):
    def __init__(self, dim: int, num_layer: int, num_heads: int, num_GTLs: int,
        ...
        token_mixer_list = [TokenMixer(dim) if _ > (num_GTLs - 1) else SWSA(dim=dim, num_heads=num_heads,
...
if __name__ == '__main__':
    ...
    net = EMT(upscale=4, dim=60, n_groups=6, n_blocks=6,num_in_ch=3,num_out_ch=3,num_heads=3,mlp_ratio=2,
              num_sa=2,window_list=[ [32, 8],[8, 32] ],shift_list=[ [16, 4],[4, 16]])
